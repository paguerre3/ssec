...
***Spring Security***
- Adding security dependency <code>implementation 'org.springframework.boot:spring-boot-starter-security'</code>
**automatically enables the login/wall Page that requires user and password** autogenerated by default }
(it changes everytime the app is restarted until proper configuration), 
which can be seen in the console during startup, e.g.:
```text
// console output:
